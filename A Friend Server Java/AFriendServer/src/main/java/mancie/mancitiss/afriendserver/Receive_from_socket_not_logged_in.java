    ...
    public void run() {
        ...
        try {
            ...
            if (data != null && !data.isEmpty()) {
                ...
                else if (instruction.equals("0013")){
                    // forgot password
                    String username = Tools.receive_Unicode_Automatically(DIS);
                    String email = Tools.receive_Unicode_Automatically(DIS);
                    // get username info
                    try(PreparedStatement cmd = Program.sql.prepareStatement("Select top 1 id from account where username = ? and email = ?") ){
                        cmd.setNString(1, username);
                        cmd.setNString(2, email);
                        try(ResultSet rs = cmd.executeQuery()){
                            if (rs.next()){
                                String id = rs.getString("id");
                                String str_id = new String(id);
                                ...
                                System.out.println(id);
                                // create a random otp containing 8 digits using SecureRandom class
                                String otp = "";
                                for (int i = 0; i < 8; i++){
                                    otp += Integer.toString(Program.secure_rand.nextInt(10));
                                }
                                // get timestamp
                                Program.otps.put(id, new OTP(otp, System.currentTimeMillis() + 300000)); // 5 minutes

                                // send email
                                try{
                                    String subject = "AFriend Chat App - You have requested to reset your password";
                                    String body = "Your OTP is " + otp + ". It will expire in 5 minutes.";
                                    EmailSender.sendEmail(email, subject, body);
                                } catch (Exception e){
                                    e.printStackTrace();
                                }
                                DOS.write("1013".getBytes(StandardCharsets.UTF_16LE));
                            } else {
                                DOS.write("1113".getBytes(StandardCharsets.UTF_16LE));
                            }
                        }
                    } catch (Exception e){
                        e.printStackTrace();
                ...
                else if (instruction.equals("1013")){
                    // reset password
                    String id = Tools.receive_Unicode_Automatically(DIS);
                    String otp = Tools.receive_Unicode_Automatically(DIS);
                    String new_pw = Tools.receive_Unicode_Automatically(DIS);
                    if (Program.otps.containsKey(id)){
                        OTP otp_obj = Program.otps.get(id);
                        if (otp_obj.otp.equals(otp)){
                            if (otp_obj.time > System.currentTimeMillis()){
                                try(PreparedStatement cmd = Program.sql.prepareStatement("update account set pw = ? where id = ?")){
                                    cmd.setNString(1, BCrypt.hashpw(new_pw, BCrypt.gensalt()));
                                    cmd.setLong(2, Long.parseLong(id));
                                    cmd.executeUpdate();
                                } catch (Exception e){
                                    e.printStackTrace();
                                }
                                DOS.write("1014".getBytes(StandardCharsets.UTF_16LE));
                            } else {
                                DOS.write("1114".getBytes(StandardCharsets.UTF_16LE));
                            }
                        } else {
                            DOS.write("1214".getBytes(StandardCharsets.UTF_16LE));
                        }
                    } else {
                        DOS.write("1314".getBytes(StandardCharsets.UTF_16LE));
                    }
                }
                else if (instruction.equals("0014")){
                    // check username
                    String username = Tools.receive_Unicode_Automatically(DIS);
                    try(PreparedStatement cmd = Program.sql.prepareStatement("select top 1 id from account where username = ?")){
                        cmd.setNString(1, username);
                        try(ResultSet rs = cmd.executeQuery()){
                            if (rs.next()){
                                DOS.write("1114".getBytes(StandardCharsets.UTF_16LE));
