                ...
                if (instruction == "0012") {
                        ...
                        try (ResultSet rs = cmd.executeQuery()){
                            if (rs.next()){
                                if (BCrypt.checkpw(lst_str[1], rs.getString("pw"))){
                                    ...
                                    String namebyte = name.getBytes(StandardCharsets.UTF_16).toString();
                                    ...
                                    DOS.write(("0200" + id + namebytelen + namebyte + name + priv).getBytes(StandardCharsets.UTF_16));
                                    System.out.println("Before dictionaries");
                                    try{
                                        if (Program.sessions.containsKey(id)){
                                            try{
                                            Program.sessions.get(id).is_locked.set(1);
                                            Program.sessions.get(id).stream.write("2004".getBytes(StandardCharsets.UTF_16));
                                                    ...
                                                    try(PreparedStatement friendCommand2 = Program.sql.prepareStatement("select top 1 id, name from account where id=?")){
                                                        ...
                                                        try(ResultSet friendResult2 = friendCommand2.executeQuery()){
                                                            if (friendResult2.next()){
                                                                ...
                                                                DOS.write(Tools.data_with_unicode_byte(datasend).getBytes(StandardCharsets.UTF_16));
                                                            }
                                                        }
                                                    }
                                                }
                                        ...
                                        if (client.loaded == 0){
                                            DOS.write(Tools.data_with_unicode_byte("2411").getBytes(StandardCharsets.UTF_16));
                                        }
                                        ...
                                        try{
                                            ...
                                            if (file.exists()){
                                                ...
                                                DOS.write(Tools.combine("0601".getBytes(StandardCharsets.UTF_16), encoded.getBytes(StandardCharsets.US_ASCII)));
                                            }
                                        ...
                                        client.stream.write(("7351" + Byte.toString(mystate)).getBytes(StandardCharsets.UTF_16));
                                    ...
                                    try{
                                        try{
                                            DOS.write("-200".getBytes(StandardCharsets.UTF_16));
                                        } catch (Exception e){
                                        }
                                        try{
                                            DIS.close();
                    ...
                    try{
                        ...
                        if (!Program.check_existed_username(lst_str[0])){
                            ...
                            DOS.write("1011".getBytes(StandardCharsets.UTF_16));
                        } else {
                            DOS.write("1111".getBytes(StandardCharsets.UTF_16));
                        }
                    } catch (Exception e){
                        e.printStackTrace();
                    } finally {
